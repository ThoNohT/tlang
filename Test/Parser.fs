...
let parserTests =
    testList
        "Parser"
        [ testCase "succeed succeeds on empty input" <| fun _ -> assertSuccessWith (P.succeed ()) "" () "" 1 0
          testCase "succeed succeeds on nonempty input and doesn't consume"
          <| fun _ -> assertSuccessWith (P.succeed ()) "x" () "x" 1 0
          testCase "fail fails on empty input" <| fun _ -> assertFailureAt (P.fail ()) "" 1 0
          testCase "fail fails on nonempty input" <| fun _ -> assertFailureAt (P.fail ()) "x" 1 0
          testCase "all succeeds on empty input and consumes everything"
          <| fun _ -> assertSuccessWith P.all "" "" "" 1 0
          testCase "all succeeds on nonempty input" <| fun _ -> assertSuccessWith P.all "abc" "abc" "" 1 3 ]

let combinatorTests =
    testList
        "Combinators"
        [ testCase "inv fails on success" <| fun _ -> assertFailureAt (P.inv (P.succeed ())) "" 1 0
          testCase "inv succeeds on failure" <| fun _ -> assertSuccessWith (P.inv (P.fail ())) "abc" () "abc" 1 0
          testCase "peek succeeds but doesn't consume"
          <| fun _ -> assertSuccessWith (P.peek P.pChar) "abc" () "abc" 1 0
          testCase "peek fails on failure" <| fun _ -> assertFailureAt (P.peek P.num) "abc" 1 0
          ...
          <| fun _ -> assertSuccessWith (P.check (fun _ -> true) P.pChar) "abc" 'a' "bc" 1 1
          testCase "sequence uses the provided combine function"
          <| fun _ -> assertSuccessWith (P.sequence (-) P.digit P.digit) "642" 2 "2" 1 2
          ...
          <| fun _ -> assertSuccessWith (P.andThen (-) P.digit P.digit) "642" -2 "2" 1 2
          ...
          <| fun _ -> assertSuccessWith (P.skipNext P.digit P.digit) "642" 6 "2" 1 2
          testCase "andThen fails when the first parser fails"
          ...
          <| fun _ -> assertSuccessWith (P.skipPrev P.digit P.digit) "642" 4 "2" 1 2
          testCase "andThen fails when the first parser fails"
          ...
          <| fun _ -> assertSuccessWith (P.map (fun x -> x + 1) P.digit) "1a" 2 "a" 1 1
          ...
          <| fun _ -> assertSuccessWith (P.alt (P.lit "abc") (P.lit "ab")) "abcd" "abc" "d" 1 3
          testCase "alt falls back to the second parser"
          <| fun _ -> assertSuccessWith (P.alt (P.lit "abc") (P.lit "abd")) "abde" "abd" "e" 1 3
          ...
          <| fun _ -> assertSuccessWith (P.oneOf [ P.lit "abc" ; P.lit "ab" ; P.lit "a" ]) "abcd" "abc" "d" 1 3
          testCase "oneOf falls back to the second parser"
          <| fun _ -> assertSuccessWith (P.oneOf [ P.lit "abc" ; P.lit "abd" ; P.lit "a" ]) "abde" "abd" "e" 1 3
          testCase "oneOf falls back to the third parser"
          <| fun _ -> assertSuccessWith (P.oneOf [ P.lit "abc" ; P.lit "abd" ; P.lit "ab" ]) "abff" "ab" "ff" 1 2
...
let regexCombinatorTests =
    testList
        ...
        [ testCase "star consumes no input when the first occurrence fails"
          <| fun _ -> assertSuccessWith (P.star P.digit) "a123" [] "a123" 1 0
          testCase "star consumes input until the first occurrence that fails"
          <| fun _ -> assertSuccessWith (P.star P.digit) "123a" [ 1 ; 2 ; 3 ] "a" 1 3
          ...
          <| fun _ -> assertSuccessWith (P.plus P.digit) "123a" [ 1 ; 2 ; 3 ] "a" 1 3
          ...
          <| fun _ -> assertSuccessWith (P.times 2 P.digit) "1234" [ 1 ; 2 ] "34" 1 2
          testCase "optional consumes no input when the first occurrence fails"
          <| fun _ -> assertSuccessWith (P.optional P.digit) "a123" None "a123" 1 0
          testCase "optional consumes input only once"
          <| fun _ -> assertSuccessWith (P.optional P.digit) "123a" (Some 1) "23a" 1 1 ]

let characterParserTests =
    testList
        "Character parsers"
        [ testCase "pChar fails on empty input" <| fun _ -> assertFailureAt P.pChar "" 1 0
          testCase "pChar succeeds on nonempty input" <| fun _ -> assertSuccessWith P.pChar "abc" 'a' "bc" 1 1
          testCase "num succeeds on a number" <| fun _ -> assertSuccessWith P.num "123" '1' "23" 1 1
          testCase "num fails with non-numeric characters"
          <| fun _ -> assertFailuresAt P.num [ "a" ; "-" ; ";" ; " " ] 1 1
          testCase "num fails on empty input" <| fun _ -> assertFailureAt P.num "" 1 0
          testCase "alpha succeeds on a letter" <| fun _ -> assertSuccessWith P.alpha "abc" 'a' "bc" 1 1
          testCase "alpha fails with non-alpha characters"
          <| fun _ -> assertFailuresAt P.alpha [ "1" ; "-" ; ";" ; " " ] 1 1
          testCase "alpha fails on empty input" <| fun _ -> assertFailureAt P.alpha "" 1 0
          testCase "alphaNum succeeds on a letter" <| fun _ -> assertSuccessWith P.alphaNum "abc" 'a' "bc" 1 1
          testCase "alphaNum succeeds on a number" <| fun _ -> assertSuccessWith P.alphaNum "123" '1' "23" 1 1
          testCase "alphaNum fails with non-alphaNumeric characters"
          <| fun _ -> assertFailuresAt P.alphaNum [ "-" ; ";" ; " " ] 1 1
          testCase "alphaNum fails on empty input" <| fun _ -> assertFailureAt P.alphaNum "" 1 0
          testCase "litC succeeds on the exact character" <| fun _ -> assertSuccessWith (P.litC 'a') "abc" 'a' "bc" 1 1
          testCase "litC fails on any other character"
          <| fun _ -> assertFailuresAt (P.litC 'a') [ "bc" ; "123" ; "f" ; " " ; ";" ] 1 1
          testCase "litC fails on empty input" <| fun _ -> assertFailureAt (P.litC 'a') "" 1 0
          testCase "litC with end of line" <| fun _ -> assertSuccessWith (P.litC '\n') "\nabc" '\n' "abc" 1 1 ]

let stringParserTests =
    testList
        "String parsers"
        [ testCase "lit succeeds on the exact input" <| fun _ -> assertSuccessWith (P.lit "abc") "abcd" "abc" "d" 1 3
          testCase "lit fails on the empty input" <| fun _ -> assertFailureAt (P.lit "abc") "" 1 0
          testCase "eoi succeeds on empty input" <| fun _ -> assertSuccessWith P.eoi "" () "" 1 0
          ...
          testCase "eol succeeds when there is no more input" <| fun _ -> assertSuccessWith P.eol "" () "" 1 0
          ...
          <| fun _ -> assertSuccessWith (P.line P.digit) "5" 5 "" 1 1
          testCase "line fails when the parser doesnt leave end of line or input"
          <| fun _ -> assertFailuresAt (P.line P.digit) [ "5a" ; "5 " ; "5-" ] 1 2
          ...
          <| fun _ -> assertSuccessWith (P.separated P.digit P.alpha) "a1b2c3d456" [ 'a' ; 'b' ; 'c' ; 'd' ] "456" 1 7
          ...
          <| fun _ -> assertSuccessWith (P.separated (P.litC ';') P.digit) "1.2.3" [ 1 ] ".2.3" 1 1
          testCase "separated_ succeeds when multiple items are separated_"
          <| fun _ -> assertSuccessWith (P.separated_ "1" P.alpha) "a1b1c1d156" [ 'a' ; 'b' ; 'c' ; 'd' ] "156" 1 7
          ...
          <| fun _ -> assertSuccessWith (P.separated_ ";" P.digit) "1.2.3" [ 1 ] ".2.3" 1 1
          testCase "stringof takes characters as long as the parser matches"
          <| fun _ -> assertSuccessWith (P.stringOf P.num) "1234abc" "1234" "abc" 1 4
          ...
          <| fun _ -> assertSuccessWith (P.stringOf_ P.num) "1234abc" "1234" "abc" 1 4
          testCase "stringOf_ succeeds when the first character doesn't match"
          <| fun _ -> assertSuccessWith (P.stringOf_ P.num) "a2134" "" "a2134" 1 0
          testCase "stringOfLength takes characters as long as the parser matches and the length is not yet reached"
          <| fun _ -> assertSuccessWith (P.stringOfLength 3 P.num) "1234abc" "123" "4abc" 1 3
          ...
          <| fun _ -> assertSuccessWith (P.takeWhile ((=) 'a')) "aaaa123" "aaaa" "123" 1 4
          ...
          <| fun _ -> assertSuccessWith (P.takeWhile_ ((=) 'a')) "aaaa123" "aaaa" "123" 1 4
          testCase "takeWhile_ succeeds when the first character doesn't match"
          <| fun _ -> assertSuccessWith (P.takeWhile_ ((=) 'a')) "baaa" "" "baaa" 1 0 ]
...
let intParserTests =
    testList
        ...
        [ testCase "parseInt returns a succeeding parser for a positive int"
          <| fun _ -> assertSuccessWith (P.parseInt "123") "" 123 "" 1 0
          testCase "parseInt returns a succeeding parser for a negative int"
          <| fun _ -> assertSuccessWith (P.parseInt "-123") "" -123 "" 1 0
          testCase "digit succeeds on a digit" <| fun _ -> assertSuccessWith P.digit "1abc" 1 "abc" 1 1
          ...
          <| fun _ -> assertSuccessWith P.nonNegativeInt "123ab" 123 "ab" 1 3
          testCase "nonNegativeInt succeeds for zero" <| fun _ -> assertSuccessWith P.nonNegativeInt "0ab" 0 "ab" 1 1
